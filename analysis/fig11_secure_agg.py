...
def _plot(dataset: str, rows: list, out_stem: str, title_suffix: str) -> None:
    ...
    for ax, e in zip(axes, _ELAPSED_GRID, strict=True):
        ...
        ax.set_xticklabels([plotting.display_name(m, short=True) for m in _GROUPS], fontsize=7, rotation=20, ha="right")
        ax.set_title(f"{dataset}, e={e}", fontsize=8)
        ax.tick_params(axis="y", labelsize=7)

    axes[0].set_ylabel("TPR@1%FPR", fontsize=8)
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, fontsize=7, loc="lower center", ncol=3, bbox_to_anchor=(0.5, 0.06))
    fig.text(0.5, 0.01, "hatched bars: F1 aggregate/global identical to full by construction (Prop. 3)", fontsize=7, ha="center")
