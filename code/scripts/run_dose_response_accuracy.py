...
METHODS = {
    "m5_hybrid_replay": (HybridReplay, "buffer_size_per_class",
                          {"local_epochs": 30, "lr": 0.5}),
    "m2_target": (TARGET, "n_synthetic_per_class",
                  {"local_epochs": 30, "lr": 0.5, "replay_ratio": 1.0}),
}
...
def main() -> int:
    ...
    rows = []
    for method_name, (method_cls, knob_name, base_cfg) in METHODS.items():
        for level in LEVELS:
            for seed in SEEDS:
                ...
                cfg = {"n_classes": n_classes, "feature_dim": feature_dim, **base_cfg, knob_name: level}
                method = method_cls(cfg)
                result = sim.run(method, X, y, stream, seed=seed, eval_sets=eval_sets)
                rows.append({
                    "dataset": DATASET, "method": method_name, "knob_name": knob_name,
                    "knob_value": level, "seed": seed,
                    "retention_bwt": result["bwt"], "final_acc": result["final_avg_acc"],
                })
                print(f"{method_name} {knob_name}={level} seed={seed}: "
                      f"final_acc={result['final_avg_acc']:.4f} bwt={result['bwt']:.4f}")

    out_csv = REPO_ROOT / "results" / "dose_response_accuracy.csv"
    ...
    config = {
        "seed": 0, "purpose": "FX8 dose-response real accuracy/BWT (claim C2, H2)",
        "dataset": DATASET, "methods": list(METHODS), "levels": LEVELS, "seeds": SEEDS,
    }
